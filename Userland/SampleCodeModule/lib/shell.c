...
int runCommandLine(int argCount, char **args)
{
  ...
  char **argv = args;
  int pid = startTask(foundCommand->runner, argc, argv, foreground, NULL);
  if (pid == -1)
  {
...
void runShell()
{
  ...
  clearScreen();
  write("Bienvendio a QUESOS\n", 21, cheeseColor);
  help(1, NULL);
  _putc('\n');
  while (1)
  {
    write("QUESOS>", 7, cheeseColor);
    char *args[MAX_ARGS];
...
int runPipeCommand(int argc, char **argv, int fdin, int fdout, int foreground)
{
  ...
  return startTask(foundCommand->runner, argc, argv, foreground, fd);
}